...
Page({
  ...
  startCountdown() {
    ...
    this.countdownTimer = setInterval(() => {
      ...
      if (count > 0) {
        ...
        this.setData({ cdText: '开始！', cdPulse: true, cdTextStyle: 'font-size:140rpx;margin-left:40rpx;' })
      } else {
  ...
  endGame() {
    ...
    const isUnlock = starCount >= 3
    const isHighScore = starCount >= 4

    this.setData({
      state: 'result', starCount, resultTitle: title,
    ...
    unlock.saveRecord('drum', {
      stars: starCount, rating: title, score,
