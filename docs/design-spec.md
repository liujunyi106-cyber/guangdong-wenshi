...
> 版本：V2.13 | 来源：Figma `📱 首页` (19:91) + `敲鼓玩法` (43:168) + `商店` (68:2) + `故事详情` (110:26) | 日期：2026-06-20 | 更新：新增涂色页规范+橡皮擦滑块
> 
> 本次更新：新增涂色页规范(§5.14)含橡皮擦大小滑块;修正§11.0 status-bar height 48→88rpx;未解锁小狮opacity 0.55→0.45;删除锁图标引用
>
...
- 超时 miss: 外圈缩至 0.4（约 1500ms，与交互文档 §4.4.1 对齐）
- 命中区高亮脉冲动画
...
│                                  │
│         ┌──────────────┐         │
│         │  IP 动作大图  │         │ ← 360×360rpx, aspectFit, 按星级映射
│         └──────────────┘         │
│                                  │
│            ⭐⭐⭐⭐⭐              │ ← 36px, 逐颗弹出 200ms/颗
...
| 状态 | 路由 | 说明 |
|------|------|------|
| `outline` | IP形象选择 | 6 IP 3×2 网格，选中后「开始涂色」 |
| `coloring` | 涂色/装饰编辑 | Canvas 画布 + 调色盘/装饰盘 + 工具栏 |
| `done` | 保存完成 | ✅已保存 + 预览 + 再画一张/分享 |

#### 5.14.1 IP选择页（outline）

| 属性 | 值 |
|------|-----|
| 标题 "狮子绘画" | 40rpx Bold #000000，居中 sub-header |
| 区域标题 "选择IP形象" | 32rpx Bold #000000 |
| IP 卡片 | 320×242rpx，32rpx 圆角，#F5F5F5 背景 |
| 卡片布局 | 2 列 3 行（每行 2 张），flex-wrap，gap 30rpx，space-between |
| 选中态 | 品牌红边框 4rpx solid #E31E15 + #FFF5F3 背景 |
| 未选中 | 透明边框 4rpx solid transparent 占位 |
| 开始涂色按钮 | 全宽 96rpx 高，48rpx 圆角，#E31E15 品牌红 |
| IP 形象图 | 140×140rpx thumb，名称 28rpx #333333 |

#### 5.14.2 涂色编辑页（coloring）

**顶部栏：**

| 属性 | 值 |
|------|-----|
| 返回按钮 | ← sub-header，白色背景页面用灰色 `#AAAAAA` |
| 分段控件 | 🎨 涂色 / 🖌️ 装饰，36rpx 字号，选中态品牌红 + 下划线 |
...
|------|------|
| ↩ 撤销 | 44×88rpx 图标，白底灰边 |
| 🧹 橡皮擦 | 44×88rpx 图标+文字标签「橡皮擦」，激活态红色边框 #E31E15 + #FFF5F3 背景；与颜色/装饰互斥 |
| 💾 保存 | 44×88rpx 图标+文字标签「保存」，#E31E15 红底白字 |

**橡皮擦大小滑块（cover-view 实现，V2.13 新增）：**

| 属性 | 值 |
|------|-----|
| 位置 | `position: fixed; right: 8rpx; top: 50%; margin-top: -220rpx` |
| 尺寸 | 64rpx 宽 × 440rpx 高 |
| 轨道 | 40rpx 宽 × 344rpx 高，#E8E8E8，圆角 8rpx |
| 滑块 | 36rpx 圆形，`#999999`，在轨道上百分比定位 |
| 标签 | 上方「粗」/ 下方「细」，24rpx，#999999 |
| 层级 | `z-index: 999`，cover-view 覆盖 Canvas |

**画布区：**

| 属性 | 值 |
|------|-----|
| canvas | `type="2d"`，width/height 100%，绝对定位 |
| 触摸层 | 透明层覆盖 canvas，bindtouchstart/move/end |
| IP 参考图 | 左上角固定，右下角红圈拖拽缩放（1x~3x） |
| 参考图边框 | 4rpx solid #E31E15，16rpx 圆角 |
| 装饰拖拽预览 | 半透明 ghost 层跟随手指 |

**画笔粗细（涂色模式，非橡皮擦）：**

| 属性 | 值 |
|------|-----|
| slider | 8 级：2/3/4/6/8/10/14/20px，默认 6px（第4档），品牌红轨道 |
| 标签 | 左「细」右「粗」，26rpx #999999 |

**调色盘（涂色模式）：**

| 属性 | 值 |
|------|-----|
| 色块 | 16 色圆形，88rpx 直径，4rpx 透明边框 |
| 选中态 | 白色/黑色描边 + box-shadow 6rpx |
| 布局 | scroll-view 左右滑动，gap 24rpx |
| 色值 | #E31E15,#F36F59,#FF5722,#E91E63,#F5A623,#FFC107,#FF9800,#FFEB3B,#65B96A,#4CAF50,#8BC34A,#5AB8E8,#2196F3,#9C27B0,#000000,#FFFFFF |

**装饰盘（装饰模式）：**

| 属性 | 值 |
|------|-----|
| 素材 | 5 类×5 个 = 25 个（祥云/火焰/蝴蝶结/花朵/星星） |
| 缩略图 | 120×120rpx，24rpx 圆角，#F5F5F5 背景 |
| 选中态 | 品牌红边框 4rpx solid #E31E15 + #FFF5F3 |
| 布局 | scroll-view 左右滑动，gap 24rpx |
...
|------|-----|
| ✅ 已保存 | 48rpx，#34C759，居中 |
| 预览图 | 600×600rpx，32rpx 圆角，#F5F5F5 背景 |
| 按钮行 | 再画一张（白底灰边）+ 分享好友（品牌红 `button open-type="share"`） |
| 按钮高度 | 96rpx，16rpx 圆角 |
| 解锁提示 | "首次完成可解锁金金狮动画哦~"，24rpx #AAAAAA |
...
### 5.16 个人中心页 (`pages/mine/mine`)

**页面结构：** `page > status-bar + scroll-view`，底部导航栏始终显示。

#### 5.16.1 用户卡片（Hero 风格，对齐商店 Hero）
...
| 未解锁 | 3D IP 形象图, **opacity 0.45（保留原色）**, 无锁图标叠加 |
