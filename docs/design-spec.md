...
| `--color-gray-dark` | `#333333` | 状态栏文字（9:41） |
...
| **Status Bar** | 14px | 500 (Medium) | Inter | "9:41" |
...
| 区域 | y 起点 | 高度 | 宽度 | 说明 |
|------|--------|------|------|------|
| 状态栏 | 0 | 44px | 375px | "9:41" 在 y=7, x=14 |
...
| 类型 | 命中区标记 | 鼓棒方向 | 音波位置 |
|------|-----------|---------|---------|
| ① 中心左面单敲 | 中心圆左侧圆形虚线 (50×50) | 左棒 -25° | 左 25% |
| ② 中心右面单敲 | 中心圆右侧圆形虚线 (50×50) | 右棒 25° | 右 75% |
| ③ 中心双面齐敲 | 中心大圆形虚线 (80×80) | 双棒同时 | 正中心 |
| ④ 边缘左/右侧单敲 | 边缘竖线标记 (24×56) | 对应侧棒 -45° | 对应边缘 |
| ⑤ 边缘两侧齐敲 | 两侧边缘标记同时亮 | 双棒同时刺左右边缘 | 正中心 |

**鼓棒样式：**
- 160×14 px, 边缘 3px 实线 #ddd, 中间透明 rgba(200,200,200,0.08)
- 动画：从屏幕外侧飞入 → 命中鼓面 → 消失, 0.4s ease-in
- 中心敲击倾斜 25°, 边缘敲击倾斜 45-55°

**音波环：**
- 3px 白色半透明边框, 从 100px→16px 向内收缩, 0.45s ease-in

**判定文字：**
- perfect 金色 #e4c14a / good 白色 / miss 灰色 #999
- 弹出动画 scale 0.5→1.3→上浮消失, 0.8s

**节奏：**
- 拍间距 800ms（原型）, 接入音频后对齐 BPM
- 超时 miss: 2500ms
- 命中区虚线框：脉冲发光动画（0.8s 交替，白色光晕 8→20px）
...
| IP 动作图 | 240×240rpx, aspectFit, 按星级映射 4 张动作图（§5.9.1） |
| 星级 | 2-5 颗, 36px, 从左到右逐颗弹出 |
| 称号 | 22px Bold #E31E15 (S:太厉害了！舞狮大师! / A:好棒呀！舞狮小能手! / B:不错哦！再练练就好了！ / C:加油加油！你可以的！) |
| 详情 | 14px #E31E15: ✨超棒/N次 👍不错/N次 💪漏掉/N次 / 最长连击/得分 |
| 按钮 | 130×48, 圆角 8px, [重试]白底 + 图标 /images/3d-icons/retry.png 同行 / [返回]黑底白字 |
| 判定语 | 儿童友好: ✨超棒!/👍不错!/😅漏掉啦 |
| 提示语 | 敲左边！/敲右边！/一起敲中间！/敲左边鼓边！/两边一起敲！ |
| 解锁触发 | 首次通关（≥B）Toast + 动作图切换
...
| 位置 | 文案 | 字号/字重 |
|------|------|-----------|
| 状态栏 | 9:41 | Inter 14px Medium, x=14,y=7 |
